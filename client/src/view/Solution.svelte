<script lang="ts">
  ...
  import type { SolutionCardCopy } from "../lib/i18n";
...
<article class="panel-surface hairline h-full rounded-[1.75rem] p-4 sm:p-5">
  <div class="flex items-start justify-between gap-3 border-b border-white/10 pb-4">
    <div class="min-w-0">
      <p class="text-[0.7rem] font-semibold uppercase tracking-[0.25em] text-ocean-300/80">{uiText.solutionIndex(index + 1)}</p>
      <h3 class="mt-1 text-lg font-semibold text-white">{solution.name}</h3>
      <p class="mt-1 text-sm text-ocean-100/70">{uiText.byPrefix} {solution.author}</p>
    </div>
    <span class="rounded-full border border-sand-200/25 bg-sand-200/10 px-3 py-1 text-xs font-medium text-sand-100">{uiText.liveTarget}</span>
  </div>

  {#if canEdit}
    <div class="mt-4 rounded-2xl border border-white/10 bg-black/15 px-3 py-3">
      <div class="flex items-center justify-between gap-3">
        <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-ocean-300/70">{uiText.editor}</p>
      ...
      {#if isEditing}
        <div class="mt-3 grid gap-3">
          ...
          <div>
            <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-ocean-200/70">{uiText.elements}</p>
          ...
          <div class="flex flex-wrap gap-2">
            <button type="button" class="touch-target rounded-2xl bg-sand-200 px-4 text-sm font-semibold text-ocean-950 transition hover:bg-sand-100 disabled:cursor-not-allowed disabled:opacity-60" onclick={onSave} disabled={busy}>
              {busy ? uiText.saving : uiText.save}
            </button>
          </div>
  ...
  <div class="mt-4 grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
    ...
    <div>
      <p class="text-xs font-semibold uppercase tracking-[0.22em] text-ocean-300/75">{uiText.ratios}</p>
