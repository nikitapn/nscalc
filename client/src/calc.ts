...
import { TableItemWithOwnership } from './table_item'
...
let el_tab = require('./elements.js');
...
export class Solution extends TableItemWithOwnership {
	...
	constructor(is_new: boolean, owner?: string) { super(is_new, owner); }

	public static create(): Solution {
		let s = new Solution(true);
		s.mx_value = "New Solution";
		s.elements = new Array<SolutionElement>(ELEMENTS_MAX);
	...
	public static create_from_data(data: NPKCalc.Flat_npkcalc.Solution_Direct): Solution {
		...
		s.elements = new Array<SolutionElement>(ELEMENTS_MAX);
		let vde = data.elements_vd();
		for (let i = 0; i < ELEMENTS_MAX; ++i) {
			s.elements[i] = new SolutionElement(i, vde.get_val(i));
		}
		return s;
	}
