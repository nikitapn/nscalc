...
public:
  SOCKS5Session(tcp::socket socket, proxy::User* proxy_user)
...
private:
  ...
  void do_read_auth_methods(uint8_t nmethods) {
    auto self = shared_from_this();
    client_socket_.async_read_some(
      asio::buffer(buffer_.data(), nmethods),
      [self](boost::system::error_code ec, std::size_t length) {
        if (ec) {
  ...
  void do_write_auth_response() {
    auto self = shared_from_this();
    asio::async_write(client_socket_, asio::buffer(buffer_.data(), 2),
  ...
  void do_read_target_address(SOCKS5_ATYP atyp) {
    ...
    if (atyp == SOCKS5_ATYP::IPV4) {
      client_socket_.async_read_some(
        asio::buffer(buffer_.data(), 6), // 4 bytes IP + 2 bytes port
        [self](boost::system::error_code ec, std::size_t length) {
          if (ec || length < 6) {
      ...
      client_socket_.async_read_some(
        asio::buffer(buffer_.data(), 1), // domain length
        [self](boost::system::error_code ec, std::size_t length) {
          if (ec || length < 1) {
          ...
          self->client_socket_.async_read_some(
            asio::buffer(self->buffer_.data(), domain_len + 2), // domain + port
  ...
  void establish_tunnel() {
    // Here we would establish the connection through the NPRPC proxy
  ...
  void send_connect_response(SOCKS5_REP rep) {
    ...
    auto self = shared_from_this();
    asio::async_write(client_socket_, asio::buffer(buffer_.data(), 10),
  ...
  void do_write_to_client(proxy::bytestream&& data) {
    ...
    auto self = shared_from_this();
    asio::async_write(client_socket_, asio::buffer(data.data(), data.size()),
      [self](boost::system::error_code ec, std::size_t) {
        if (ec) {
...
public:
  SOCKS5Server(asio::io_context& ioc, unsigned short port, proxy::User* proxy_user)
    : ioc_(ioc)
...
public:
    ...
    std::string_view secret)
  {
    try {
      ...
      user_ = dynamic_cast<proxy::User*>(user_obj);
